...
const ProjectDetail = () => {
    ...
    const confirmCriteria = async () => {
        ...
        try {
            // Send gender criteria
            await fetch(`/api/save_criteria_gender/${projectId}/`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ gender: tempSelectedCriteria.gender }),
            });
    
            // Send age group criteria
            await fetch(`/api/save_criteria_age_group/${projectId}/`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ age_group: tempSelectedCriteria.ageGroup }),
            });
    
            // Send interest criteria
            await fetch(`/api/save_criteria_interest/${projectId}/`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ interest: tempSelectedCriteria.interest }),
            });
    
            setSuccess("Project criteria updated successfully!");
            setShowAddCriteriaModal(false);
        } catch (error) {
            setError("Failed to update project criteria. Please try again.");
        }
    };
